...
public class Profil implements Serializable{
    ...
    public float calculateBasalMetabolism(User user){
        if(user.getGender() == "female"){
            ...
            return (float) (65.5 + (13.7 * (user.getWeight())) + (5 * user.getHeight()) - (6.7 * user.getAge()));
        }
        
    }
    
    public float calculateBodyMassIndex(User user){
        return (float) (user.getWeight()/pow(user.getHeight(), 2));
    }
    ...
    public void getRecipeNumbersByLabelAndDate(int id, String label){
        ...
        for(int i = 0; i < 7; i++){
            ...
            System.out.println(weeklyTriedRecipe[i]);
    ...
    public static void main(String[] args) {
        Profil p = new Profil();
        p.getRecipeNumbersByLabelAndDate(1, "vegan");
